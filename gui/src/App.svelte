<script lang="ts">
  ...
  let spawnTaskId = $state<string | null>(null)
  let spawnInitialPrompt = $state('')
  let agentIdInput: HTMLInputElement | undefined

  // Beads state
  let beadsAvailable = $state(false)
  let readyTasks = $state<BeadsTask[]>([])
  ...
  onMount(async () => {
    ...
    // Check Beads availability and load tasks
    try {
      beadsAvailable = await invoke('beads_available')
      if (beadsAvailable) {
        await refreshTasks()
      }
    } catch (e) {
      console.warn('Could not check beads:', e)
    }
  })

  async function refreshTasks() {
    try {
      readyTasks = await invoke('get_ready_tasks')
    } catch (e) {
  ...
  // Sanitize agent name for git branch compatibility
  ...
  async function spawnAgent() {
    console.log('spawnAgent called', { spawnAgentId, spawnCommand, spawnWorkdir, spawnIsolated, spawnBaseBranch, spawnTaskId })
    ...
    isSpawning = true
    try {
      // Sanitize and generate unique agent ID like CLI does: {base}-{random}
      const sanitizedName = sanitizeAgentName(spawnAgentId.trim())
      if (!sanitizedName) {
        alert('Agent name must contain at least one letter or number')
        isSpawning = false
        return
      }
      const uniqueAgentId = `${sanitizedName}-${generateIdSuffix()}`
      console.log('Calling invoke spawn_agent with unique ID:', uniqueAgentId)

      // Pass initial prompt via -p flag so Claude starts immediately
      const promptToSend = spawnInitialPrompt.trim() || null
      ...
      // Reset spawn dialog state
      spawnTaskId = null
      spawnInitialPrompt = ''
      // Refresh tasks since one was claimed
      if (beadsAvailable) {
        await refreshTasks()
      }
      await refreshSessions()
  ...
  <div class="dialog-overlay" onclick={() => showSpawnDialog = false} role="presentation">
    <div class="dialog" onclick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" tabindex="-1">
      ...
      <form class="form-fields" onsubmit={(e) => { e.preventDefault(); spawnAgent(); }}>
        ...
        {#if beadsAvailable && readyTasks.length > 0}
          <label>
            <span>Assign Task (optional)</span>
            <select
              bind:value={spawnTaskId}
              class="task-select"
              onchange={(e) => {
                const taskId = (e.target as HTMLSelectElement).value
                if (taskId) {
                  const task = readyTasks.find(t => t.id === taskId)
                  if (task) {
                    // Auto-populate initial prompt with task info
                    spawnInitialPrompt = `Work on task ${task.id}: ${task.title}${task.description ? `\n\nDescription: ${task.description}` : ''}`
                  }
                } else {
                  spawnInitialPrompt = ''
                }
            ...
            >
              ...
              {#each readyTasks as task}
                <option value={task.id}>
                  P{task.priority ?? '?'} [{task.id}] {task.title.slice(0, 40)}{task.title.length > 40 ? '...' : ''}
                </option>
              {/each}
            </select>
            {#if spawnTaskId}
              {@const selectedTask = readyTasks.find(t => t.id === spawnTaskId)}
              {#if selectedTask}
                <span class="field-hint task-hint">P{selectedTask.priority ?? '?'} · {selectedTask.issue_type ?? 'task'}</span>
              {/if}
            {/if}
          </label>
        {/if}
        ...
        <label>
          ...
          <textarea
            bind:value={spawnInitialPrompt}
            placeholder="Enter a prompt to send to the agent after it starts..."
            rows="3"
            class="initial-prompt"
          ></textarea>
          <span class="field-hint">Will be sent automatically after agent initializes</span>
        </label>
