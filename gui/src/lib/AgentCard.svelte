...
>
  ...
  <div class="meta-row">
    ...
    {#if session.branch}
      <span class="branch" title={session.branch}>
        ⎇ {session.branch.replace('rembrandt/', '')}
      </span>
    {/if}
  </div>

  {#if session.task_id}
    <div class="task-row" title={session.task_title || session.task_id}>
      <span class="task-id">{session.task_id}</span>
      {#if session.task_title}
        <span class="task-title">{session.task_title.slice(0, 30)}{session.task_title.length > 30 ? '...' : ''}</span>
      {/if}
    </div>
  {/if}
